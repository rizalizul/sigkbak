...
const buildKBAKPopup = (p) => {
    ...
    <div style="font-family:'Inter',sans-serif;background:white;border-radius:12px;overflow:hidden;width:280px;">
      <div style="padding:14px 16px 12px;background:${cfg.color}18;border-bottom:3px solid ${cfg.color};">
        ...
        <span style="display:inline-block;font-size:11px;font-weight:600;padding:3px 10px;border-radius:20px;background:${cfg.color};color:white;">Level ${level}</span>
      </div>
...
export const KBAKLayer = ({ visible }) => {
    ...
    const addLayer = () => {
        ...
        layerRef.current = L.geoJSON(dataRef.current, {
            style: (f) => {
            ...
            onEachFeature: (feature, layer) => {
                ...
                layer.on("click", function () {
                    // Auto zoom ke bounding box poligon
                ...
                layer.on("mouseover", function () {
                    this.setStyle({ weight: 2.5, fillOpacity: Math.min((KBAK_LEVELS[parseInt(feature.properties?.KLSKBAK) || 1] || KBAK_LEVELS[1]).fillOpacity + 0.2, 0.8) });
                });
    ...
    const removeLayer = () => {
        if (layerRef.current) {
            mapHook.removeLayer(layerRef.current);
